...
class Adult : public Person{
    ...
    public:
        ...
        void displayInfo() override{
            ...
            for(int i = 0; i < NUM_ADULT_BORROW; i++){
                if(code[i] != 0) {
                    cout << setw(6) << code[i];
                }
            }
        ...
        void removeBook(int c) override{
            ...
            for (int i = idxToRemove; i < NUM_ADULT_BORROW - 1; i++) {
                code[i] = code[i + 1];
                code[i + 1] = 0;
            }
        ...
        void sort_code() override{
            ...
            for (i = 0; i < NUM_ADULT_BORROW - 1; i++){
                if (code[i] == 0) continue;
                min_idx = i;
                for (j = i + 1; j < NUM_ADULT_BORROW; j++){
